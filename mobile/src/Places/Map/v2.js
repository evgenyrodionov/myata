...
import {
  StyleSheet, View, Dimensions, Animated,
} from 'react-native';
...
const ASPECT_RATIO = (screen.width / screen.height) * 0.7;
...
class AnimatedViews extends React.Component {
  constructor(props) {
    ...
    const animations = this.props.places.map((m, i) =>
    ...
    region
      .timing({
        latitude: scrollX.interpolate({
          inputRange: places.map((m, i) => i * SNAP_WIDTH),
          outputRange: places.map(m => m.address.lat),
        }),
        longitude: scrollX.interpolate({
          inputRange: places.map((m, i) => i * SNAP_WIDTH),
          outputRange: places.map(m => m.address.lon),
        }),
        duration: 0,
      })
      .start();
  ...
  onPanYChange = ({ value }) => {
    const {
      canMoveHorizontal, region, scrollY, scrollX, index,
    } = this.state;
    ...
    if (shouldBeMovable !== canMoveHorizontal) {
      ...
      if (!shouldBeMovable) {
        const { address } = this.props.places[index];
        ...
        region
          .timing({
            latitude: scrollY.interpolate({
              ...
              outputRange: [
                address.lat,
                address.lat - LATITUDE_DELTA * 0.5 * 0.375,
              ],
              extrapolate: 'clamp',
            }),
        ...
        region
          .timing({
            latitude: scrollX.interpolate({
              inputRange: this.props.places.map((m, i) => i * SNAP_WIDTH),
              outputRange: this.props.places.map(m => m.address.lat),
            }),
            longitude: scrollX.interpolate({
              inputRange: this.props.places.map((m, i) => i * SNAP_WIDTH),
              outputRange: this.props.places.map(m => m.address.lon),
            }),
            duration: 0,
          })
          .start();
  ...
  render() {
    ...
    const {
      panX, panY, animations, canMoveHorizontal, region,
    } = this.state;
          ...
          <View style={styles.itemContainer}>
            {places.map(
              (
                ...
                const {
                  translateY, translateX, scale, opacity,
                } = animations[
                  i
                ];
